function [tform valid_pair_num error] = getRigidTransform(new_pointcloud, ref_pointcloud, ref_normals)
    ...
    for iter = 1:iter_num
        ...
        [validM, validN] = find(assoc_pts(:,:,1) ~= 0);
        valid_pair_num = numel(validM);
        % Write your code here...
        for i=1:1:valid_pair_num
            mVal = validM(i);
            nVal = validN(i);
            N = ref_normals(mVal, nVal, :);
            N = reshape(N, [3,1]);
            Vn = reshape(assoc_pts(mVal, nVal,:), [3,1]);
            Vref = reshape(ref_pts(mVal, nVal,:), [3,1]);
            skewMat = toSkewSym(Vn);
            G = [skewMat, eye(3)];
            A(mVal*(n-1) + nVal,:) = N'*G;
            b(mVal*(n-1) + nVal,:) = N'*(Vref - Vn);
        end
